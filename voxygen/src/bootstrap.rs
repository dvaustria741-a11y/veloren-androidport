...
//!
//! This is the body that used to live directly in `main.rs`'s `fn main()`.
//! It's pulled out here so both the desktop entry point (`main.rs`, which
//! only adds CLI parsing) and the Android entry point
//! (`crate::android::android_main`) can share it, instead of duplicating
//! ~150 lines of settings/audio/window/global-state setup. Anything that
//! genuinely differs per platform (event loop construction) is handled
//! inside `Window::new` / `crate::android`, not here.
...
pub fn bootstrap_and_run(args: cli::Args) {
    ...
    let mut i18n =
        i18n::LocalizationHandle::load(&settings.language.selected_language).unwrap_or_else(
            |error| {
                let selected_language = &settings.language.selected_language;
                warn!(
                    ?error,
                    ?selected_language,
                    "Impossible to load language: change to the default language (English) \
                     instead.",
                );
                i18n::REFERENCE_LANG.clone_into(&mut settings.language.selected_language);
                i18n::LocalizationHandle::load_expect(&settings.language.selected_language)
            },
        );
    i18n.set_english_fallback(settings.language.use_english_fallback);

    // Create window. Event loop construction is platform-specific and handled
    // inside `Window::new` (see `crate::android::build_event_loop` for Android).
